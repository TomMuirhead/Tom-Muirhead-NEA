...
namespace NEA_Game
{
	class Unit
	{
		...
		public Unit(object sender,GameControl baseControl, Map currentMap, Point tileCoord)//Create a sprite on the map at chosen tile location
		{
			...
			map.mapBox.Controls.Add(unitBox);
		...
		public Point GetTileCoord(PictureBox sender)
		{
			int x = sender.Left;
			int y = sender.Top;
